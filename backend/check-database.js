...
async function checkDatabase() {
  try {
    ...
    users.forEach(user => {
      console.log(
        `  - ${user.first_name} ${user.last_name} (${user.email}) - ${user.role} ${user.is_verified ? '✅' : '❌'}`
      );
    });
    ...
    const trainerProfiles = await db('trainer_profiles')
      ...
      .select(
        'users.first_name',
        'users.last_name',
        'trainer_profiles.location',
        'trainer_profiles.hourly_rate',
        'trainer_profiles.is_featured'
      );
    console.log('\n👨‍💼 Trainer Profiles:');
    trainerProfiles.forEach(trainer => {
      console.log(
        `  - ${trainer.first_name} ${trainer.last_name} (${trainer.location}) - $${trainer.hourly_rate}/hr ${trainer.is_featured ? '⭐ Featured' : ''}`
      );
    });
    ...
    const trainerSpecialties = await db('trainer_specialties')
      .join(
        'trainer_profiles',
        'trainer_specialties.trainer_id',
        'trainer_profiles.id'
      )
      .join('users', 'trainer_profiles.user_id', 'users.id')
    ...
    // Check session types
    const sessionTypes = await db('session_types').select(
      'name',
      'duration_minutes',
      'base_price'
    );
    ...
    sessionTypes.forEach(session => {
      console.log(
        `  - ${session.name}: ${session.duration_minutes}min - $${session.base_price}`
      );
    });
    ...
    const trainerCount = await db('trainer_profiles')
      .count('* as count')
      .first();
    const sessionTypeCount = await db('session_types')
      .count('* as count')
      .first();
