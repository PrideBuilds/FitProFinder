...
async function seedDatabase() {
  try {
    ...
    const specialties = [
      ...
      {
        name: 'Rehabilitation',
        description: 'Injury recovery and physical therapy',
        icon: 'healing',
      },
    ];
    ...
    const johnUser = await db('users')
      .where('email', 'john.trainer@example.com')
      .first();
    const sarahUser = await db('users')
      .where('email', 'sarah.yoga@example.com')
      .first();
    ...
    const trainerProfiles = [
      ...
      },
      {
        ...
        certifications:
          'RYT-500 Yoga Alliance, Meditation Teacher Certification',
        years_experience: 5,
        is_verified: true,
        is_featured: false,
      },
    ...
    const johnProfile = await db('trainer_profiles')
      .where('user_id', johnUser.id)
      .first();
    const sarahProfile = await db('trainer_profiles')
      .where('user_id', sarahUser.id)
      .first();
    const personalTraining = await db('specialties')
      .where('name', 'Personal Training')
      .first();
    const strengthTraining = await db('specialties')
    ...
    const sessionTypes = [
      ...
      {
        name: 'Yoga Session',
        description: 'Individual yoga instruction',
        duration_minutes: 60,
        base_price: 60.0,
      },
      {
      ...
      {
        name: 'Group Training',
        description: 'Small group fitness session',
        duration_minutes: 60,
        base_price: 40.0,
      },
    ];
