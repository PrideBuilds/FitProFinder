export const seed = async knex => {
  ...
  const trainers = await knex('trainer_profiles').select(
    'user_id',
    'business_name'
  );
  ...
  for (const trainer of trainers) {
    const trainerSessionTypes = [
      ...
      },
      {
        ...
        description:
          'Short consultation to discuss your fitness goals and create a plan.',
        duration_minutes: 30,
        price: 45.0,
        type: 'individual',
        max_participants: 1,
  ...
  const sessionTypesFromDb = await knex('session_types').select(
    'id',
    'trainer_id',
    'duration_minutes',
    'price'
  ...
  if (clients.length > 0 && sessionTypesFromDb.length > 0) {
    ...
    for (let i = 0; i < Math.min(3, clients.length); i++) {
      ...
      sampleBookings.push({
        ...
        meeting_link:
          i % 2 === 1 ? 'https://meet.google.com/sample-link' : null,
        status: 'confirmed',
