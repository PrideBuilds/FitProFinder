...
export const seed = async (knex) => {
  ...
  const existingSarah = await knex('users').where({ email: 'trainer@demo.com' }).first();
  if (existingSarah) {
  ...
  const trainerSpecialties = [
    // Mike's specialties
    { trainer_id: trainerUsers[0].id, specialty_id: 1, is_primary: true, experience_years: 8 }, // Personal Training
    { trainer_id: trainerUsers[0].id, specialty_id: 4, is_primary: false, experience_years: 6 }, // Strength Training
    { trainer_id: trainerUsers[0].id, specialty_id: 8, is_primary: false, experience_years: 5 }, // Sports Performance
    { trainer_id: trainerUsers[0].id, specialty_id: 9, is_primary: false, experience_years: 4 }, // HIIT

    // Emma's specialties
    { trainer_id: trainerUsers[1].id, specialty_id: 2, is_primary: true, experience_years: 6 }, // Yoga
    { trainer_id: trainerUsers[1].id, specialty_id: 5, is_primary: false, experience_years: 4 }, // Pilates

    // Alex's specialties
    { trainer_id: trainerUsers[2].id, specialty_id: 4, is_primary: true, experience_years: 10 }, // Strength Training
    { trainer_id: trainerUsers[2].id, specialty_id: 1, is_primary: false, experience_years: 8 }, // Personal Training
    { trainer_id: trainerUsers[2].id, specialty_id: 8, is_primary: false, experience_years: 7 }, // Sports Performance

    // Sarah's specialties (existing demo user)
    { trainer_id: existingSarah?.id, specialty_id: 3, is_primary: true, experience_years: 5 }, // Nutrition
    { trainer_id: existingSarah?.id, specialty_id: 1, is_primary: false, experience_years: 3 }, // Personal Training
    { trainer_id: existingSarah?.id, specialty_id: 7, is_primary: false, experience_years: 4 }, // Weight Loss
  ...
  console.log('- mike.fitness@demo.com / demo123456 (Strength & Sports Performance)');
  console.log('- emma.yoga@demo.com / demo123456 (Yoga & Pilates)');
  console.log('- alex.strength@demo.com / demo123456 (Powerlifting & Strength)');
