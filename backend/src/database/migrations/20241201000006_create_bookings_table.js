export const up = async (knex) => {
  await knex.schema.createTable('bookings', (table) => {
    ...
    table.string('session_type_id', 36).notNullable().references('id').inTable('session_types').onDelete('CASCADE');
    
    // Booking details
    table.timestamp('scheduled_at').notNullable();
    table.integer('duration_minutes').notNullable();
    table.string('session_format').defaultTo('in_person').checkIn(['online', 'in_person']);
    table.text('location_details'); // for in-person sessions
    table.text('meeting_link'); // for online sessions
    
    // Status tracking
    table.string('status').defaultTo('pending').checkIn(['pending', 'confirmed', 'in_progress', 'completed', 'cancelled', 'no_show']);
    ...
    // Payment tracking
    table.string('payment_status').defaultTo('pending').checkIn(['pending', 'paid', 'refunded', 'partially_refunded']);
