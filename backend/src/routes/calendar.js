...
router.get('/integration/status', authenticate, async (req, res, next) => {
  try {
    ...
    if (!trainer) {
      return next(new ApiError('Only trainers can access calendar integration', 'TRAINER_ONLY', 403));
    }
    ...
    next(new ApiError('Failed to get calendar integration status', 'INTEGRATION_STATUS_ERROR', 500));
  }
});
...
router.get('/connect/google', authenticate, async (req, res, next) => {
  try {
    ...
    const trainer = await db('trainer_profiles').where('user_id', trainerId).first();
    if (!trainer) {
      return next(new ApiError('Only trainers can connect calendar integration', 'TRAINER_ONLY', 403));
    }
    ...
    next(new ApiError('Failed to generate authorization URL', 'AUTH_URL_ERROR', 500));
  }
});
...
router.get('/oauth/callback', async (req, res, next) => {
  try {
    ...
    if (error) {
      return res.redirect(`${process.env.FRONTEND_URL}/dashboard/trainer?calendar_error=${encodeURIComponent(error)}`);
    }

    if (!code || !trainerId) {
      return res.redirect(`${process.env.FRONTEND_URL}/dashboard/trainer?calendar_error=missing_parameters`);
    }
    ...
    if (result.success) {
      res.redirect(`${process.env.FRONTEND_URL}/dashboard/trainer?calendar_connected=true`);
    } else {
      res.redirect(`${process.env.FRONTEND_URL}/dashboard/trainer?calendar_error=connection_failed`);
    }
...
router.patch('/integration/settings', authenticate, async (req, res, next) => {
  try {
    ...
    if (!integration) {
      return next(new ApiError('Calendar integration not found', 'INTEGRATION_NOT_FOUND', 404));
    }
    ...
    if (create_events_in_external !== undefined) updateData.create_events_in_external = create_events_in_external;
    if (import_events_from_external !== undefined) updateData.import_events_from_external = import_events_from_external;
    if (sync_interval_minutes !== undefined) updateData.sync_interval_minutes = sync_interval_minutes;
    if (event_title_template !== undefined) updateData.event_title_template = event_title_template;
    if (event_description_template !== undefined) updateData.event_description_template = event_description_template;
    if (event_visibility !== undefined) updateData.event_visibility = event_visibility;
    if (include_client_contact !== undefined) updateData.include_client_contact = include_client_contact;
    ...
    next(new ApiError('Failed to update calendar integration settings', 'SETTINGS_UPDATE_ERROR', 500));
  }
});
...
router.delete('/integration', authenticate, async (req, res, next) => {
  try {
    ...
    next(new ApiError('Failed to disconnect calendar', 'DISCONNECT_ERROR', 500));
  }
});
...
router.post('/test', authenticate, async (req, res, next) => {
  try {
    ...
    if (!integration) {
      return next(new ApiError('No active calendar integration found', 'NO_INTEGRATION', 404));
    }
