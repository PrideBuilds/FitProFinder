...
router.get('/availability/:trainerId', async (req, res, next) => {
  try {
    ...
    const trainer = await db('trainer_profiles').where('user_id', trainerId).first();
    if (!trainer) {
    ...
    if (date) {
      ...
      query = query
        .where('slots.recurrence_start_date', '<=', end.toISOString().split('T')[0])
        .andWhere(function() {
          this.whereNull('slots.recurrence_end_date')
            .orWhere('slots.recurrence_end_date', '>=', start.toISOString().split('T')[0]);
        });
    }

    const availabilitySlots = await query.select(
      'slots.*'
    );
    ...
    for (const slot of availabilitySlots) {
      if (date) {
        ...
        const slotTimes = generateTimeSlotsForDate(slot, new Date(date), sessionDuration);
        ...
        for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
          ...
          if (slot.day_of_week === dayOfWeek) {
            const slotTimes = generateTimeSlotsForDate(slot, new Date(d), sessionDuration);
            timeSlots.push(...slotTimes);
          }
        }
      }
    ...
    const availableSlots = timeSlots.filter(slot => 
      !bookedTimes.has(new Date(slot.start_time).toISOString())
    );
    ...
    res.json({
      ...
      data: {
        trainerId,
        date: date || null,
        dateRange: date ? null : { start: startDate || new Date(), end: endDate },
    ...
    next(new ApiError('Failed to fetch availability', 'AVAILABILITY_ERROR', 500));
  }
});
...
router.get('/session-types/:trainerId', async (req, res, next) => {
  try {
    ...
    next(new ApiError('Failed to fetch session types', 'SESSION_TYPES_ERROR', 500));
  }
});
...
router.post('/', authenticate, validateRequest([
  ...
  try {
    ...
    if (!sessionType) {
      await trx.rollback();
      return next(new ApiError('Invalid session type', 'INVALID_SESSION_TYPE', 400));
    }

    // Validate session format is allowed
    if (sessionFormat === 'online' && !sessionType.allows_online) {
      await trx.rollback();
      return next(new ApiError('Online sessions not available for this session type', 'ONLINE_NOT_ALLOWED', 400));
    }

    if (sessionFormat === 'in_person' && !sessionType.allows_in_person) {
      await trx.rollback();
      return next(new ApiError('In-person sessions not available for this session type', 'IN_PERSON_NOT_ALLOWED', 400));
    }
    ...
    if (existingBooking) {
      await trx.rollback();
      return next(new ApiError('Time slot is no longer available', 'TIME_SLOT_TAKEN', 409));
    }
    ...
    const [bookingId] = await trx('bookings').insert({
      ...
      location_details: sessionFormat === 'in_person' ? locationDetails : null,
      status: 'pending',
    ...
    next(new ApiError('Failed to create booking', 'BOOKING_CREATION_ERROR', 500));
  }
});
...
router.get('/my-bookings', authenticate, async (req, res, next) => {
  try {
    ...
    if (startDate) {
      query = query.where('b.scheduled_at', '>=', new Date(startDate).toISOString());
    }
    if (endDate) {
      query = query.where('b.scheduled_at', '<=', new Date(endDate).toISOString());
    }
    ...
    if (startDate) totalQuery.where('b.scheduled_at', '>=', new Date(startDate).toISOString());
    if (endDate) totalQuery.where('b.scheduled_at', '<=', new Date(endDate).toISOString());
    if (upcoming === 'true') totalQuery.where('b.scheduled_at', '>=', new Date().toISOString());
    else if (upcoming === 'false') totalQuery.where('b.scheduled_at', '<', new Date().toISOString());
...
router.patch('/:bookingId/status', authenticate, validateRequest(['status']), async (req, res, next) => {
  try {
    ...
    if (!booking) {
      return next(new ApiError('Booking not found', 'BOOKING_NOT_FOUND', 404));
    }
    ...
    next(new ApiError('Failed to update booking status', 'BOOKING_UPDATE_ERROR', 500));
  }
});
