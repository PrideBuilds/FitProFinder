...
router.get('/', [
  ...
  const formattedTrainers = trainers.map(trainer => ({
    ...
    specialtyIds: trainer.specialty_ids ? trainer.specialty_ids.split(',').map(id => parseInt(id)) : []
  }));
...
router.get('/:id', asyncHandler(async (req, res) => {
  ...
  let socialLinks = {};
  try {
    socialLinks = trainer.social_links ? JSON.parse(trainer.social_links) : {};
  } catch (error) {
    socialLinks = {};
  }
 ...
 */
router.post('/profile', authenticate, authorize('trainer'), [
  ...
  try {
    ...
    const existingProfile = await trx('trainer_profiles').where('user_id', userId).first();
    ...
    if (existingProfile) {
      ...
      await trx('trainer_profiles').where('user_id', userId).update(profileData);
    } else {
      // Create new profile
