...
export const validateConfig = () => {
  ...
  if (config.isProduction) {
    if (!config.supabase.url)
      errors.push('PUBLIC_SUPABASE_URL is required in production');
    if (!config.supabase.anonKey)
      errors.push('PUBLIC_SUPABASE_ANON_KEY is required in production');
    if (!config.cometchat.appId)
      errors.push('PUBLIC_COMETCHAT_APP_ID is required in production');
  ...
  if (config.features.messaging && !config.cometchat.appId) {
    errors.push(
      'PUBLIC_COMETCHAT_APP_ID is required when messaging is enabled'
    );
  }
