...
const DEMO_TRAINERS: Trainer[] = [
  {
    ...
    experienceYears: 8,
    certifications: [
      { name: "NASM-CPT", organization: "NASM", dateObtained: new Date("2016-03-01") },
      { name: "Nutrition Coach", organization: "NASM", dateObtained: new Date("2018-06-01") }
    ],
    location: {
  ...
  // Add some additional trainers for search variety
  {
    ...
    lastName: "Rodriguez",
    role: "trainer",
    profileImageUrl: "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=150&h=150&fit=crop&crop=face",
...
function filterTrainers(trainers: Trainer[], filters: Partial<SearchFilters>): Trainer[] {
  return trainers.filter(trainer => {
    // City filter
    if (filters.city && !trainer.location.city.toLowerCase().includes(filters.city.toLowerCase())) {
      return false;
    }

    // State filter
    if (filters.state && !trainer.location.state.toLowerCase().includes(filters.state.toLowerCase())) {
      return false;
    }
...
function paginateTrainers(trainers: Trainer[], page: number = 1, limit: number = 9): SearchResult {
...
export default function TrainerSearch({ initialFilters = {} }: TrainerSearchProps) {
  ...
  const [filters, setFilters] = useState<Partial<SearchFilters>>(getFiltersFromURL());
  ...
  const loadTrainers = async (currentFilters: Partial<SearchFilters>) => {
    ...
    try {
      ...
      const results = paginateTrainers(filteredTrainers, currentFilters.page || 1, currentFilters.limit || 9);
      ...
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-red-50 border border-red-200 rounded-lg p-6">
            <h3 className="text-lg font-medium text-red-800 mb-2">Error Loading Trainers</h3>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            ...
            {/* Show active filters */}
            {(filters.city || filters.state || filters.specialties?.length || filters.rating || filters.verified) && (
              <span className="ml-2">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              ...
              {filters.specialties?.map((specialty, index) => (
                <span key={index} className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                  {specialty}
                  <button
                    onClick={() => handleFilterChange({ specialties: undefined })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <input
                ...
                value={filters.city && filters.state ? `${filters.city}, ${filters.state}` : filters.city || ''}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                onChange={(e) => {
                  const [city, state] = e.target.value.split(',').map(s => s.trim());
                  handleFilterChange({ city: city || undefined, state: state || undefined });
                }}
              />
            </div>
            ...
            <div>
              ...
              <label className="flex items-center">
                ...
                />
                <span className="ml-2 text-sm text-gray-600">Show verified trainers only</span>
              </label>
            </div>
          </div>
        </div>
      ...
      <div className="flex space-x-2">
        ...
        <button 
          onClick={() => {
            ...
            alert(`Message feature coming soon! You can contact ${trainer.firstName} ${trainer.lastName} at their profile page.`);
          }}
