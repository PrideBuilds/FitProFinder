...
class PreDeploymentAudit {
  ...
  checkCriticalFiles() {
    ...
    const criticalFiles = [
      ...
      {
        path: 'backend/src/services/messagingService.js',
  ...
  checkPackageFiles() {
    ...
    try {
      ...
      const backendPkg = JSON.parse(
        fs.readFileSync(path.join(rootDir, 'backend/package.json'))
      );
      ...
      try {
        ...
        if (jwtMatch && jwtMatch[1]) {
          ...
          if (secret.length < 32) {
            ...
            this.log(
              'warning',
              'JWT_SECRET is using default value - change for production'
            );
          } else {
      ...
      if (fs.existsSync(fullPath)) {
        try {
          ...
          if (content.includes('import ') && content.includes('require(')) {
            this.log(
              'warning',
              `Mixed imports in ${description}`,
              'Use either ES6 imports or CommonJS, not both'
            );
          }
  ...
  checkSecurityIssues() {
    ...
    const checkSecurityInFile = (filePath, description) => {
      ...
      if (fs.existsSync(fullPath)) {
        ...
        if (
          content.match(/password\s*=\s*['"]\w+['"]/i) &&
  ...
  generateReport() {
    ...
    if (this.errors.length === 0) {
      ...
      console.log(
        '🛑 NOT ready for deployment. Please fix critical errors first.'
      );
      return false;
    }
  }
