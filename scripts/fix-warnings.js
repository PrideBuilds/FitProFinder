...
const fixJWTSecret = () => {
  ...
  if (fs.existsSync(envPath)) {
    ...
    if (
      envContent.includes('your-super-secret-jwt-key-change-this-in-production')
    ) {
  ...
  if (fs.existsSync(authPath)) {
    ...
    const filteredLines = lines.filter(line => {
      ...
      if (
        ...
        (trimmed.includes('🔍') ||
          trimmed.includes('Creating token') ||
          trimmed.includes('Using JWT_SECRET'))
      ) {
        modified = true;
        return false;
      }
    ...
    if (modified) {
      ...
      console.log(
        '✅ Removed debug console.log statements from auth utilities'
      );
    } else {
...
try {
  ...
  console.log('\n📝 Next steps:');
  console.log(
    '   1. Run the audit again: node scripts/pre-deployment-audit.js'
  );
  console.log('   2. Follow the deployment guide: deploy-guide.md');
  console.log(
    '   3. Test locally before deploying: npm run build && cd backend && npm start'
  );
} catch (error) {
